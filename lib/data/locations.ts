...
export function normalizeStateName(rawState: string): string {
  if (!rawState) return "";
  const normalizedRaw = rawState.toLowerCase().replace(/[^a-z]/g, "");
  
  for (const standardState of INDIA_STATES) {
    const normalizedStandard = standardState.toLowerCase().replace(/[^a-z]/g, "");
    if (normalizedRaw.includes(normalizedStandard)) {
...
export function normalizeCityName(rawCity: string, stateName: string): string {
  if (!rawCity || !stateName) return rawCity || "";
  const districts = INDIA_STATES_AND_DISTRICTS[stateName] || [];
  const normalizedRaw = rawCity.toLowerCase().replace(/[^a-z]/g, "");
  
  for (const standardCity of districts) {
    const normalizedStandard = standardCity.toLowerCase().replace(/[^a-z]/g, "");
